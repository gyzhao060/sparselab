...
subroutine calc_cost(&
  ...
  if (isfcv .eqv. .True.) then
    ...
    cost = cost + chisq
    call daxpy(Npix,1d0,gradchisq(1:Npix),1,gradcost(1:Npix),1)
  end if
...
subroutine chisq_fcv(Freal,Fimag,Vreal,Vimag,&
  ...
  do ifcv=1, Nfcv
    ...
    ! calc chi-square
    chisq = chisq + (resid1*resid1+resid2*resid2)/Varfcv(ifcv)/Nfcv
    
    ! calc gradient of chi-square
    factor1 = 2/Varfcv(ifcv)*resid1/Nfcv
    factor2 = 2/Varfcv(ifcv)*resid2*sign(1,uvidxfcv(ifcv))/Nfcv
...
subroutine chisq_amp(gradVamp,Vreal,Vimag,&
  ...
  do iamp=1, Namp
    ...
    ! calc chi-square
    chisq = chisq + resid*resid/Varamp(iamp)/Namp
    
    ! calc gradient of chisquare
    !  using BLAS
    call daxpy(Npix,2/Varamp(iamp)*resid/Namp,&
...
subroutine chisq_cphase(gradVpha,Vreal,Vimag,&
  ...
  do icp=1, Ncp
    ...
    ! calc chi-square
    chisq = chisq + resid*resid/Varcp(icp)/Ncp
    
    ! calc gradient of chi-square
    factor=2/Varcp(icp)*resid/Ncp
    do ibl=1,3
      uvidx = abs(uvidxcp(ibl,icp))
...
subroutine chisq_camp(gradVamp,Vreal,Vimag,&
  ...
  do ica=1, Nca
    ...
    ! calc chi-square
    chisq = chisq + resid*resid/Varca(ica)/Nca
    
    ! calc gradient of chisquare
    factor1=2/Varca(ica)*resid/Nca
    do ibl=1,4
      uvidx = abs(uvidxca(ibl,ica))
...
subroutine model_amp(gradVamp,Vreal,Vimag,&
  ...
  do iamp=1, Namp
    ...
    ! calc chi-square
    chisq = chisq + Vampres(iamp)*Vampres(iamp)/Varamp(iamp)/Namp
    
    ! calc gradient of chisquare
    !  using BLAS
    call daxpy(Npix,2/Varamp(iamp)*Vampres(iamp)/Namp,&
               gradVamp(1:Npix,uvidx),1,gradchisq(1:Npix),1)
    !  not using BLAS
    !gradchisq(1:Npix) = gradchisq(1:Npix) &
    !                  + 2/Varamp(iamp)*resid*gradVamp(1:Npix,uvidx)
